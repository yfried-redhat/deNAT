...
def calc_wtime(wts):
    ...
    frac_num = decimal.Decimal(frac_num)
    while frac_num >= 1:
        frac_num = decimal.Decimal(frac_num/10)
    return decimal.Decimal(int_num + frac_num)


class dPacket(object):
    def __init__(self, wtime, dstip, src_port,
                 dst_port, IPid, TCPts, sn=None):
#         captured time
        self.sn = sn
        self.time = wtime
#         packet Serial Number within session
    ...
    def attr_val_as_list(self):
        return [self.time, self.dst_ip, self.sport,
                self.dport, self.IPid, self.TCPts]
...
def parse_packet(header, data):
    ...
    eth = impacket.ImpactDecoder.EthDecoder().decode(data)
    ip = eth.child()
    tcp = ip.child()
...
class dStream(object):
    ...
    def get_filter(self):
        cap_filter = ('((tcp.srcport == {sport}) && (tcp.dstport == {dport}) '
                      '&& (ip.dst == {ip}))').format(sport=self.sport,
                                                     dport=self.dport,
                                                     ip=self.dst_ip)
        return cap_filter
...
def main(pcap_session_filename):
    pcap_session = pcapy.open_offline(pcap_session_filename)
